...
// {{ .Service }}ServerWrapper wraps the service and handles request and response logic
...
func Register{{ .Service }}ServiceWithGofr(app *gofr.App, srv {{ .Service }}ServiceWithGofr) {
	registerServerWithGofr(app, srv, func(s grpc.ServiceRegistrar, srv any) {
		wrapper := &{{ .Service }}ServerWrapper{server: srv.({{ .Service }}ServiceWithGofr), healthServer: getOrCreateHealthServer()}
...
//
// {{ $.Service }}GoFrService defines the gRPC service implementation.
